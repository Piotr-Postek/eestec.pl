...
function normalizeItems(items: FeaturedRibItem[]): FeaturedRibItem[] {
  return items.map((ev) => ({
    ...ev,
    titleEn: ev.titleEn ?? "",
...
type Props = {
  initial: FeaturedEventsPayload;
};

export function FeaturedEventsEditor({ initial }: Props) {
  ...
  const [items, setItems] = useState<FeaturedRibItem[]>(() =>
    normalizeItems(initial.items)
  );
  ...
  function updateItem(i: number, patch: Partial<FeaturedRibItem>) {
    setItems((prev) =>
  ...
  async function uploadImage(i: number, file: File) {
    ...
    try {
      ...
      setMessage("Zdjęcie wgrane — zapisz sekcję, aby zachować zmiany.");
    } catch {
    ...
    <div className="mx-auto max-w-3xl px-4 py-8">
      <div className="mb-8">
        <h1 className="text-xl font-bold tracking-tight md:text-2xl">
          Nasze główne wydarzenia
        </h1>
        <p className="mt-1 text-sm text-[var(--muted)]">
          Sekcja u dołu strony — cztery kafelki („żebra”). Muszą być dokładnie{" "}
          {FEATURED_RIBS_COUNT}.
        </p>
      </div>
      ...
      <div className="mb-6 grid gap-4 rounded-xl border border-white/[0.08] bg-[#0e1219] p-5">
        ...
        <label className="flex flex-col gap-1 text-sm">
          <span className="text-[rgba(244,246,249,0.75)]">Etykieta — EN</span>
          <input
            value={eyebrowEn}
      ...
      <div className="space-y-5">
        {items.map((ev, i) => (
          ...
          >
            <span className="mb-4 block text-xs font-semibold uppercase tracking-wider text-[var(--accent)]">
              Kafelek {i + 1} / {FEATURED_RIBS_COUNT}
            </span>
            <div className="grid gap-3 sm:grid-cols-2">
              ...
              <div className="sm:col-span-2 grid gap-3">
                ...
                <label className="flex flex-col gap-1 text-sm">
                  ...
                  <input
                    value={ev.imageUrl}
                    onChange={(e) =>
                      updateItem(i, { imageUrl: e.target.value })
                    }
              ...
              <p className="sm:col-span-2 text-xs font-medium uppercase tracking-wider text-[rgba(244,246,249,0.45)]">
                Angielski (opcjonalnie)
              </p>
              <label className="flex flex-col gap-1 text-sm sm:col-span-2">
      ...
      <p className="mt-10 text-xs leading-relaxed text-[rgba(244,246,249,0.4)]">
        Dane w pliku{" "}
        <code className="text-[rgba(244,246,249,0.55)]">
          data/featured-events.json
        </code>
        .
      </p>
    </div>
